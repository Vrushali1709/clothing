...
class Category(models.Model):
    ...
    slug = models.SlugField(max_length=255, unique=True)  # Limit 50 thi vadhari 255 kari
    gender = models.CharField(max_length=20, choices=GENDER_CHOICES, default='women')
    image = models.ImageField(upload_to='categories/', null=True, blank=True)
...
class Product(models.Model):
    category = models.ForeignKey(Category, related_name='products', on_delete=models.CASCADE)
    name = models.CharField(max_length=250)
    slug = models.SlugField(max_length=255, unique=True, null=True, blank=True)  # Limit 255 kari
    description = models.TextField()
    fabric_care = models.TextField(help_text="e.g. 100% Organic Cotton. Dry Clean Only.", null=True, blank=True)
    price = models.DecimalField(max_digits=10, decimal_places=2)
    original_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="For Discount display")
    stock = models.IntegerField(default=10)
    sizes = models.CharField(max_length=255, default="S, M, L, XL", help_text="Comma separated: S, M, L, XL")  # Limit 255 kari
    colors = models.CharField(max_length=255, default="Black, White", help_text="Comma separated: Black, White, Beige")  # Limit 255 kari
    is_featured = models.BooleanField(default=False)
    is_new_arrival = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)

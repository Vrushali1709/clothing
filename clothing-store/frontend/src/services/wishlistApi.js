...
//     // Wishlist અને Products બંને સાથે Fetch કરીને Data Merge કરીએ છીએ
//     Promise.all([
//       getWishlist(),
//       API.get('products/')
//     ])
...
//         // Products Map બનાવીએ છીએ ID વડે Fast Lookup માટે
//         const productsMap = {};
//         rawProducts.forEach(p => {
//           productsMap[p.id] = p;
//         });

//         // Wishlist items ને સાચા પ્રોડક્ટ ડેટા સાથે જોડીએ છીએ
...
//                     onError={(e) => { e.target.src = "https://placehold.co/600x800?text=Garment"; }}
//                   />
//                   <button
...
//                   <p className="font-serif text-xs sm:text-sm font-semibold text-neutral-900 mt-1.5 sm:mt-2">₹{Number(item.product.price).toLocaleString()}</p>
//                 </div>
//               </div>
...
export default function Wishlist() {
  ...
  return (
    <div className="bg-[#FAF8F5] min-h-screen text-neutral-900 pt-8 sm:pt-12 md:pt-16 pb-20 font-sans">
      <div className="max-w-[1440px] mx-auto px-6 sm:px-8 md:px-12">
        
        {/* Page Header with proper alignment and spacing */}
        <div className="mb-10 sm:mb-12 pb-6 border-b border-neutral-200 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <span className="text-[10px] uppercase tracking-[0.35em] text-neutral-400 font-bold block mb-1.5">
              Curated Favorites
            </span>
            <h1 className="text-3xl sm:text-4xl font-serif tracking-tight">Saved Wishlist</h1>
          </div>
        ...
        {/* Wishlist Grid with consistent responsive padding/margins */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
          {wishlistItems.map((item) => (
            <div key={item.wishlistId} className="bg-white border border-neutral-200/80 group flex flex-col justify-between shadow-sm hover:shadow-md transition rounded-xl overflow-hidden">
              <div>
                <div className="relative aspect-[3/4] overflow-hidden bg-neutral-100">
                  <img
                    src={getImageUrl(item.product.image)}
                  ...
                  <button
                    onClick={() => handleRemove(item.wishlistId)}
                    className="absolute top-3 right-3 p-2.5 bg-white/90 backdrop-blur rounded-full shadow-md hover:bg-red-50 text-neutral-600 hover:text-red-600 active:scale-95 transition"
                    title="Remove"
                  >
                ...
                <div className="p-4 sm:p-5">
                  <h3 className="font-serif text-xs sm:text-sm font-medium text-neutral-900 truncate">{item.product.name}</h3>
                  <p className="text-[10px] uppercase tracking-widest text-neutral-400 mt-1 truncate">{item.product.category_name}</p>
                  <p className="font-serif text-xs sm:text-sm font-semibold text-neutral-900 mt-2">₹{Number(item.product.price).toLocaleString('en-IN')}</p>
                </div>
              </div>
              ...
              <div className="p-4 pt-0">
                <button
                  onClick={() => navigate(`/product/${item.product.id}`)}
                  className="w-full bg-neutral-900 text-white py-3 text-[10px] font-semibold uppercase tracking-[0.2em] hover:bg-black active:scale-[0.98] transition"
                >
                  View Piece
                </button>
              </div>

...
const COLLECTIONS = [
  { title: 'The Monochrome Edit', subtitle: 'Minimal • Refined • Timeless', image: 'https://images.unsplash.com/photo-1483985988355-763728e1935b?q=80&w=1200', link: '/shop?collection=monochrome' },
  { title: 'Weekend Essentials', subtitle: 'Effortless Everyday Luxury', image: 'https://images.unsplash.com/photo-1529139574466-a303027c1d8b?q=80&w=1200', link: '/shop?collection=weekend' },
  { title: 'Occasion Edit', subtitle: 'Made For Your Moments', image: 'https://images.unsplash.com/photo-1515372039744-b8f02a3ae446?q=80&w=1200', link: '/shop?collection=occasion' },
];

const FAQS = [
  { question: 'What is your return policy?', answer: 'We offer a 14-day return and exchange window on eligible products. Items should be unused and returned with their original packaging and tags.' },
  { question: 'How long does delivery take?', answer: 'Orders are generally delivered within 3–7 business days depending on your location. Express delivery may be available for selected pin codes.' },
  { question: 'How can I find my correct size?', answer: 'Every eligible product includes a detailed size guide. We recommend checking the measurements before placing your order.' },
  { question: 'Do you offer Cash on Delivery?', answer: 'Cash on Delivery can be available for selected locations and order values. Availability is shown during checkout.' },
];
...
export default function Home() {
    ...
    >
      <div className="relative aspect-[3/4] bg-neutral-100 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-3 right-3 w-9 h-9 bg-white/95 backdrop-blur rounded-full flex items-center justify-center text-neutral-700 hover:scale-110 transition-transform shadow-md"
        >
          <Heart size={15} className={wishlistMap[product.id] ? 'fill-red-500 text-red-500' : 'text-neutral-600'} />
        </button>
        <span className="absolute top-3 left-3 px-2.5 py-1 bg-white/95 text-[9px] uppercase tracking-wider font-bold" style={{ color: DARK }}>
          New
        </span>
      </div>
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-2">
          <p className="text-sm font-serif font-bold" style={{ color: '#8A6D46' }}>
            ₹{Number(product.price).toLocaleString('en-IN')}
          </p>
          <div className="flex items-center gap-1 text-[10px] text-neutral-500">
            <Star size={11} fill={GOLD} style={{ color: GOLD }} />
  ...
  return (
    <div className="bg-[#FAF8F5] text-neutral-900 font-sans antialiased selection:bg-neutral-900 selection:text-white overflow-hidden">
          ...
          {HERO_SLIDES.map((_, idx) => (
            <React.Fragment key={idx}>
              <span
                ...
                style={
                  idx === currentSlide
                    ? { fontWeight: 700, color: GOLD, borderBottom: `1px solid ${GOLD}`, paddingBottom: '4px' }
                    : { color: 'rgba(255,255,255,0.4)' }
                }
              >
                0{idx + 1}
              </span>
              {idx < HERO_SLIDES.length - 1 && <div className="w-[1px] h-6 bg-white/20" />}
            </React.Fragment>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 sm:px-8 md:px-14 lg:px-20 py-16 max-w-[1550px] mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-serif">Shop By Category</h2>
          <div className="w-10 h-[1px] mx-auto mt-4" style={{ backgroundColor: GOLD }} />
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-6">
          {CATEGORIES.map((cat) => (
            <div
              ...
              className="flex flex-col items-center gap-3 cursor-pointer group"
            >
              <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden p-1 border shadow-sm transition-all group-hover:shadow-md" style={{ borderColor: '#E5DDD0' }}>
                <img src={cat.img} alt={cat.name} className="w-full h-full object-cover rounded-full group-hover:scale-105 transition-transform duration-700" />
              </div>
              <span className="text-xs font-medium tracking-widest uppercase mt-1">{cat.name}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 sm:px-8 md:px-14 lg:px-20 py-12 max-w-[1550px] mx-auto">
        <div className="flex items-end justify-between border-b border-neutral-200 pb-5 mb-10">
          <h2 className="text-3xl font-serif">Trending Pieces</h2>
          <button onClick={() => navigate('/shop')} className="hidden sm:block text-[10px] uppercase tracking-[0.2em] font-bold hover:text-[#C8A882] transition-colors">
            View All Collection →
          </button>
        </div>
        {trendingProducts.length > 0 ? (
          ...
          <div className="py-20 text-center text-neutral-400 text-sm">Loading collection...</div>
        )}
      </section>
      ...
      <section className="my-20 bg-[#EBE5DC] border-y border-neutral-200">
        <div className="grid grid-cols-1 md:grid-cols-2">
          <div className="h-[400px] sm:h-[550px] overflow-hidden">
            <img src="https://images.unsplash.com/photo-1489987707025-afc232f7ea0f?q=80&w=1400" alt="Philosophy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-1000" />
          </div>
          <div className="flex items-center p-8 sm:p-14 lg:p-20">
            <div className="max-w-xl">
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif mb-6 leading-tight">Crafted For <br /> Enduring Elegance</h2>
              <p className="text-xs sm:text-sm text-neutral-600 font-light leading-relaxed mb-8">
                Every garment we create is an ode to refined luxury, using carefully selected fabrics and uncompromising attention to detail to match your distinctive lifestyle.
              </p>
              <div className="space-y-3 mb-8">
                {['Premium fabric selection', 'Thoughtful craftsmanship', 'Timeless silhouettes'].map((item) => (
                  <div key={item} className="flex items-center gap-3 text-xs uppercase tracking-wider text-neutral-700 font-medium">
                    <Check size={14} style={{ color: GOLD }} />
                    {item}
                  </div>
                ))}
              </div>
              <button onClick={() => navigate('/about')} className="bg-neutral-900 text-white px-8 py-4 text-[10px] uppercase tracking-[0.25em] font-medium hover:bg-[#C8A882] transition-colors">
                Discover Our Story
              </button>
            </div>
          </div>
      ...
      <section className="px-5 sm:px-8 md:px-14 lg:px-20 py-12 max-w-[1550px] mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-serif">Curated Collections</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {COLLECTIONS.map((col) => (
            <div key={col.title} onClick={() => navigate(col.link)} className="relative h-[450px] overflow-hidden cursor-pointer group rounded-xl">
              <img src={col.image} alt={col.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
                <h3 className="text-2xl font-serif mb-1">{col.title}</h3>
                <p className="text-xs uppercase tracking-wider text-white/70 mb-4">{col.subtitle}</p>
                <span className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-bold" style={{ color: '#E7D3B8' }}>
                  Explore Edit <ArrowRight size={13} />
                </span>
              </div>
            </div>
      ...
      <section className="my-20 bg-[#F1ECE5] py-20 px-5">
        <div className="max-w-[1100px] mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl font-serif mb-12">Loved By Our Community</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Aarav Mehta', text: 'Beautiful quality and excellent finishing. The shirt looks even better in person.' },
              { name: 'Riya Shah', text: 'The entire shopping experience feels premium. Packaging and quality were exceptional.' },
              { name: 'Kabir Patel', text: 'The fit is perfect and delivery was much faster than expected. Will definitely shop again.' },
            ].map((review) => (
              <div key={review.name} className="bg-white p-8 shadow-sm rounded-xl text-left flex flex-col justify-between">
                <div>
                  <div className="flex gap-1 mb-4">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star key={s} size={12} fill={GOLD} style={{ color: GOLD }} />
                    ))}
                  </div>
                  <p className="text-sm font-serif leading-relaxed text-neutral-700">“{review.text}”</p>
                </div>
                <div className="mt-6 pt-4 border-t border-neutral-100">
                  <p className="text-xs uppercase tracking-widest font-bold text-neutral-900">{review.name}</p>
                  <p className="text-[10px] text-neutral-400 mt-0.5">Verified Customer</p>
                </div>
              </div>
            ))}
      ...
      <section className="px-5 sm:px-8 md:px-14 lg:px-20 py-16 max-w-[950px] mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-serif">Frequently Asked Questions</h2>
        </div>
        <div className="border-t border-neutral-200">
          {FAQS.map((faq, idx) => (
            <div key={faq.question} className="border-b border-neutral-200">
              <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} className="w-full flex items-center justify-between py-6 text-left">
                ...
                <ChevronDown size={18} className={`transition-transform duration-300 ${openFaq === idx ? 'rotate-180' : ''}`} style={{ color: GOLD }} />
              </button>
              {openFaq === idx && (
                <div className="pb-6 pr-8">
                  <p className="text-xs sm:text-sm text-neutral-500 leading-relaxed font-light">{faq.answer}</p>
                </div>
              )}
            </div>
          ))}
      ...
      <section className="px-5 sm:px-8 md:px-14 lg:px-20 py-16">
        <div className="max-w-[1550px] mx-auto bg-neutral-900 text-white py-16 px-6 text-center rounded-2xl">
          <Mail size={24} className="mx-auto mb-4" style={{ color: GOLD }} />
          <h2 className="text-3xl sm:text-4xl font-serif mb-3">Join The Atelier Private List</h2>
          <p className="text-xs sm:text-sm text-neutral-400 max-w-md mx-auto mb-8 font-light leading-relaxed">
            Be the first to discover new collections, private sales, and exclusive seasonal edits.
          </p>
          <div className="flex flex-col sm:flex-row max-w-md mx-auto gap-3">

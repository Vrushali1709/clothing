...
export default function Shop() {
  ...
  const matchesCollection = (product) => {
    ...
    if (backendCollection) {
      const normalizedBackendCollection = String(backendCollection)
        .toLowerCase()
        .replace(/\s+/g, '-');
    ...
    <div className="bg-[#FAF8F5] min-h-screen py-10 sm:py-14">
      <div className="container mx-auto px-4 sm:px-6 md:px-12 max-w-7xl">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          <h1 className="text-3xl sm:text-4xl font-serif text-neutral-900 capitalize mb-3">
            {pageTitle}
          </h1>

          {activeCollection && (
            <p className="text-xs uppercase tracking-widest text-neutral-500 mb-3 font-light">
              {activeCollection.subtitle}
            </p>
          )}

          <div className="w-12 h-[1px] bg-[#C8A882] mx-auto" />

          {activeCollection && (
            <button
              onClick={clearCollection}
              className="mt-4 inline-flex items-center gap-1.5 text-xs font-medium text-neutral-500 hover:text-neutral-900 transition underline underline-offset-4"
            >
        ...
        <div className="max-w-md mx-auto mb-8 relative">
          ...
          <input
            ...
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search garments, shirts, dresses..."
            className="w-full bg-white border border-neutral-200 pl-11 pr-10 py-3.5 text-xs tracking-wide focus:outline-none focus:border-neutral-900 transition rounded-xl shadow-sm"
          />
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-8">
          {/* Size Filter */}
          <div className="flex items-center gap-2 flex-wrap justify-center">
            <span className="text-xs text-neutral-500 font-medium mr-1">Size:</span>
            {['', 'S', 'M', 'L', 'XL'].map((size) => (
              <button
                key={size || 'all'}
                onClick={() => setSelectedSize(size)}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="bg-white border border-neutral-200 px-3 py-2 text-xs text-neutral-800 focus:outline-none focus:border-neutral-900 transition rounded-lg shadow-sm cursor-pointer"
            >
              <option value="default">Featured / Default</option>
              <option value="low-high">Price: Low to High</option>
              <option value="high-low">Price: High to Low</option>
        ...
        {/* GENDER TABS */}
        <div className="flex justify-start sm:justify-center items-center gap-6 mb-10 border-b border-neutral-200 pb-4 overflow-x-auto scrollbar-none">
          {['all', 'women', 'men', 'kids', 'ethnic'].map((gender) => (
            <button
              key={gender}
              onClick={() => handleGenderChange(gender)}
              className={`text-xs uppercase tracking-widest font-semibold transition-all pb-1 whitespace-nowrap shrink-0 ${
                currentGender === gender && !currentCategory && !currentCollection
        ...
        {/* ACTIVE FILTER INFO */}
        {(currentCollection || currentCategory || currentGender !== 'all' || searchQuery || selectedSize) && (
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6 text-xs text-neutral-500">
            <div className="flex flex-wrap gap-2">
              {currentCollection && (
                <span className="px-3 py-1 bg-neutral-900 text-white rounded-md text-[10px] uppercase tracking-wider">
                  {activeCollection?.title}
                </span>
              )}
              {currentCategory && (
                <span className="px-3 py-1 bg-white border border-neutral-200 rounded-md text-[10px] uppercase tracking-wider">
                  {currentCategory}
                </span>
              )}
              {currentGender !== 'all' && (
        ...
        {/* PRODUCT GRID */}
        {loading ? (
          <div className="text-center py-20 font-serif text-neutral-400 text-sm">
            Curating collection...
          </div>
        ) : filteredProducts.length === 0 ? (
          <div className="text-center py-20">
            <p className="font-serif text-neutral-600 text-base mb-4">
              No clothing pieces found matching your criteria.
            </p>
            {currentCollection && (
              <button
                onClick={clearCollection}
                className="inline-flex items-center gap-2 bg-neutral-900 text-white px-6 py-3 text-xs uppercase tracking-wider font-semibold hover:bg-black transition rounded-xl"
              >
                ...
                <div className="relative aspect-[3/4] bg-neutral-100 overflow-hidden">
                  <img
                    ...
                    onError={(e) => {
                      e.currentTarget.onerror = null;
                      e.currentTarget.src =
                        'https://placehold.co/600x800?text=Unavailable';
                    }}
                  />
                  ...
                  <button
                    onClick={(e) => handleWishlistToggle(e, product.id)}
                    className="absolute top-3 right-3 w-9 h-9 bg-white/95 backdrop-blur rounded-full flex items-center justify-center text-neutral-700 hover:scale-110 transition-transform shadow-md z-20"
                    title={wishlistMap[product.id] ? 'Remove from Wishlist' : 'Add to Wishlist'}
                  >
                    <Heart
                      size={15}
                ...
                <div className="p-4 flex flex-col justify-between flex-1">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-neutral-400 mb-1">
                      {product.category_name || product.category?.name || 'Atelier Collection'}
                    </p>
                    <h3 className="text-xs sm:text-sm font-medium text-neutral-800 truncate">
                      {product.name || 'Garment'}
                    </h3>
                  </div>

                  <div className="mt-3 pt-3 border-t border-neutral-100 flex items-center justify-between">
                    <span className="text-sm font-serif font-bold text-[#8A6D46]">
